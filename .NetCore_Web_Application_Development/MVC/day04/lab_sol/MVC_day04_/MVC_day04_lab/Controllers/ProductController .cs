...
namespace MVC_day04_lab.Controllers
{
    public class ProductController : Controller
    {
        ...
        public IActionResult CreateProduct(ProductVM productCreateVM)
        {
            ...
            db.Products.Add(productToDomain);
            db.SaveChanges();
            return RedirectToAction("Index");
        }
        [HttpGet]
        ...
        public IActionResult CheckName(string PName)
        {
            var isExist = db.Products.Any(p => p.Name == PName);
            if (isExist)
            {
                return Json($"Name {PName} already exist");
            }
            return Json(false);
